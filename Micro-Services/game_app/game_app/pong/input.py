    ...
    def apply_inputs_to_game(self, game):
        for input in self.input_queue:
            ...
            if input.input_id == g.INPUT_NEUTRAL:
                ...
                player.velocity.x = g.PADDLE_SPEED
            elif input.input_id == g.INPUT_SPACE:
                if game.status == g.STATUS_ACTIVE or game.status == g.STATUS_PAUSED:
                    game.status = g.STATUS_PAUSED if game.status == g.STATUS_ACTIVE else g.STATUS_ACTIVE
                elif game.status == g.STATUS_ENDED:
                    game.reset()
